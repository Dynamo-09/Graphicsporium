...
const ContactForm = ({ externalData }) => {
  ...
  const handleDesignChange = (e) => {
    setFormData({ ...formData, designType: e.target.value, websiteType: '', price: '', timeline: '' });
  };
  ...
  const handleWebsiteChange = (e) => {
    ...
    let newPrice = '';
    let newTimeline = '';
    if (wType === 'Static') { newPrice = '₹3999 - ₹17999'; newTimeline = '3 weeks'; }
    else if (wType === 'Dynamic') { newPrice = '₹19999 - ₹69999'; newTimeline = '8 weeks'; }
    else if (wType === 'E-commerce') { newPrice = '₹79999 - onwards'; newTimeline = '15 weeks'; }
    setFormData({ ...formData, websiteType: wType, price: newPrice, timeline: newTimeline });
  };

  const handleLogoPriceChange = (e) => {
    const val = e.target.value;
    let newTimeline = '';
    if (val === '1200') newTimeline = '3 days';
    else if (val === '3000') newTimeline = '4 days';
    else if (val === '5000') newTimeline = '6 days';
    else if (val === '8000') newTimeline = '12 days';
    else if (val === '10000') newTimeline = '15 days';
    setFormData({ ...formData, price: `₹${val}`, timeline: newTimeline });
  };

  const handleCustomPriceChange = (e) => {
    const rawVal = e.target.value.replace('₹', '').trim();
    const fullVal = rawVal ? `₹${rawVal}` : '';
    setFormData({ ...formData, price: fullVal });
    if (formData.designType === 'Website') {
      let expected = '';
      if (formData.websiteType === 'Static') expected = '₹3999 - ₹17999';
      else if (formData.websiteType === 'Dynamic') expected = '₹19999 - ₹69999';
      else if (formData.websiteType === 'E-commerce') expected = '₹79999 - onwards';
      if (fullVal !== expected && expected !== '') {
        setErrors({ ...errors, price: 'please follow the range' });
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('An error occurred: ' + (err?.text || err?.message || JSON.stringify(err)));
    }
    ...
    <section id="contact" className="section bg-surface-light">
      <div className="container">
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="contact-form-extended glass">
          <input type="hidden" name="Name" value={`${formData.firstName} ${formData.middleName} ${formData.lastName}`.trim()} />
          <input type="hidden" name="Contact" value={formData.contactNo} />
          <input type="hidden" name="Email" value={formData.email} />
          <input type="hidden" name="City" value={formData.city} />
          <input type="hidden" name="DesignType" value={formData.designType} />
          <input type="hidden" name="WebsiteType" value={formData.websiteType} />
          ...
          <div className="form-row">
            <div className="form-group">
              <label>Design Type *</label>
              <select required value={formData.designType} disabled>
                <option value="" disabled>Select Design Type</option>
                <option value="Logo Design">Logo Design</option>
                <option value="Website">Website</option>
              </select>
            ...
            {formData.designType === 'Website' && (
              <div className="form-group animate-fade-in">
                <label>Website Type *</label>
                <select required value={formData.websiteType} disabled>
                  <option value="" disabled>Select Website Type</option>
                  <option value="Static">Static</option>
                  <option value="Dynamic">Dynamic</option>
                  <option value="E-commerce">E-commerce</option>
            ...
            <div className="form-group">
              <label>Price *</label>
              {formData.designType === 'Logo Design' ? (
                <select required value={formData.price ? formData.price.replace('₹', '') : ''} disabled>
                  <option value="" disabled>Select Price</option>
                  <option value="1200">₹1200</option>
                  <option value="3000">₹3000</option>
                  <option value="5000">₹5000</option>
                ...
                <>
                  <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                    <span style={{ position: 'absolute', left: '1rem', color: 'var(--color-text)', pointerEvents: 'none' }}>₹</span>
                    <input type="text" required disabled value={formData.price ? formData.price.replace('₹', '') : ''} placeholder="Price" style={{ paddingLeft: '2rem', width: '100%' }} />
                  </div>
                  {errors.price && <span className="error-text" style={{ color: '#FF5A00', fontSize: '0.8rem', marginTop: '4px' }}>{errors.price}</span>}
                </>
              )}
            </div>
            <div className="form-group">
              <label>Timeline *</label>
              <input type="text" required disabled value={formData.timeline} placeholder="Timeline" />
            </div>
          </div>
...
const Footer = ({ currentLogo, currentMsmeLogo }) => {
  return (
        ...
        <div className="new-footer-grid">
          ...
          <div className="footer-col">
            ...
            <ul className="footer-links">
              ...
              <li><a className="contact-link" href="https://wa.me/917044495506" target="_blank" rel="noopener noreferrer">WhatsApp Direct</a></li>
            </ul>
          </div>
...
function WhatsAppButton() {
  return (
    <a className="whatsapp" href="https://wa.me/917044495506" target="_blank" rel="noopener noreferrer" aria-label="Chat on WhatsApp">
...
function App() {
  ...
  return (
    <div className="app">
      ...
      <Footer currentLogo={currentLogo} currentMsmeLogo={currentMsmeLogo} />
      <WhatsAppButton />
    </div>
  );
}
