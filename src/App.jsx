...
const ServiceModal = ({ isOpen, onClose, title, onAddToCart }) => {
  ...
  return (
    <div className="service-modal-overlay" style={{ 
      position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.8)', zIndex: 9999, 
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem',
      animation: isClosing ? 'fadeOutOverlay 0.3s ease-out forwards' : 'fadeInOverlay 0.3s ease-out'
    }} onClick={handleClose}>
      <div className="service-modal-content glass" style={{ position: 'relative', display: 'flex', flexDirection: 'row', width: '100%', maxWidth: '900px', backgroundColor: 'var(--color-surface)', borderRadius: '1.5rem', overflow: 'hidden', minHeight: '400px' }} onClick={(e) => e.stopPropagation()}>
        <div className="service-modal-text" style={{ flex: 1, padding: '3rem 2.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <div className="service-modal-image-placeholder" style={{ flex: 1, backgroundColor: 'rgba(255,255,255,0.03)', borderLeft: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
          {serviceImages[title] ? (
            <img src={serviceImages[title]} alt={title} style={{ width: '100%', height: '100%', objectFit: 'contain', borderRadius: '1rem' }} />
          ) : (
      ...
      <button className="close-btn" onClick={handleClose} style={{ marginTop: '1.5rem', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '50%', width: '50px', height: '50px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#fff', transition: 'all 0.3s' }}>
        <X size={28} />
      </button>
    </div>
  );
