...
import {
  ...
  Film, TrendingUp, Star, Sun, Moon, ShoppingCart
} from 'lucide-react';
...
import clientLogo10 from './assets/for web/Aavisha.jpeg';
...
// --- Navigation ---
export const Navbar = ({ isDarkTheme, toggleTheme, currentLogo, onNavClick, onToggleConsultation, cartItemCount = 0 }) => {
    ...
    <nav className={`navbar ${isScrolled ? 'scrolled' : ''}`}>
      <div className="container nav-container">
        ...
        <ul className="nav-links desktop-only">
          ...
          <li><Link to="/about" onClick={() => { setMobileMenuOpen(false); if (onNavClick) onNavClick(); }}>About Us</Link></li>
          <li><Link to="/services" onClick={() => { setMobileMenuOpen(false); if (onNavClick) onNavClick(); }}>Services</Link></li>
          <li><Link to="/portfolio" onClick={() => { setMobileMenuOpen(false); if (onNavClick) onNavClick(); }}>Portfolio</Link></li>
          <li><Link to="/blog" onClick={() => { setMobileMenuOpen(false); if (onNavClick) onNavClick(); }}>Blogs</Link></li>
        </ul>

        <div className="nav-actions desktop-only">
          <button 
            style={{ position: 'relative', background: 'transparent', border: 'none', cursor: 'pointer', padding: '0.5rem', display: 'flex', alignItems: 'center' }} 
            onClick={(e) => handleNavClick(e, '#contact')}
          >
            <ShoppingCart size={24} stroke="var(--color-primary)" />
            {cartItemCount > 0 && (
              <span style={{ position: 'absolute', top: '0', right: '0', background: '#ff4d4d', color: 'white', fontSize: '0.75rem', fontWeight: 'bold', width: '18px', height: '18px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {cartItemCount}
              </span>
            )}
          </button>
          <a href="#contact" className="btn btn-primary" onClick={(e) => handleNavClick(e, '#contact')}>Let's Talk</a>
        ...
        <div className="mobile-toggle-wrapper mobile-only" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <button 
            ...
            onClick={(e) => handleNavClick(e, '#contact')}
          >
            <ShoppingCart size={22} stroke="var(--color-primary)" />
            {cartItemCount > 0 && (
              <span style={{ position: 'absolute', top: '-4px', right: '-4px', background: '#ff4d4d', color: 'white', fontSize: '0.7rem', fontWeight: 'bold', width: '16px', height: '16px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {cartItemCount}
              </span>
            )}
          </button>
      ...
      {mobileMenuOpen && (
        <div className="mobile-menu glass">
          <ul className="mobile-nav-links">
            ...
            <li><Link to="/portfolio" onClick={() => { setMobileMenuOpen(false); if (onNavClick) onNavClick(); }}>Portfolio</Link></li>
            <li><Link to="/blog" onClick={() => { setMobileMenuOpen(false); if (onNavClick) onNavClick(); }}>Blogs</Link></li>
            <li><a href="#contact" className="btn btn-primary" onClick={(e) => handleNavClick(e, '#contact')}>Let's Talk</a></li>
          </ul>
        </div>
      )}
    </nav>
    ...
    <header id="home" className="hero-new-banner banner">
      ...
      <div className="gp-content">
        ...
        <div className="gp-cta-row">
          <Link to="/portfolio" className="gp-btn-main">View Our Portfolio</Link>
...
const StatusBar = () => {
  ...
  const companies = [clientLogo1, clientLogo2, clientLogo3, clientLogo4, clientLogo5, clientLogo6, clientLogo7, clientLogo8, clientLogo9, clientLogo10];
                ...
                {companies.map((logo, idx) => (
                  <span key={idx} className="company-logo">
                    <img 
                      ...
                      style={logo.includes('Aavisha') ? { border: '4px solid #ff5a00', borderRadius: '12px', boxSizing: 'border-box' } : {}}
                    />
                  </span>
                ))}
              </div>
  ...
  const renderPackageContent = (pkg, isSubPackage = false) => (
    <>
      {isSubPackage ? (
        ...
        <h3 style={{ borderBottom: '2px solid var(--color-primary)', paddingBottom: '0.5rem', marginBottom: '1rem', textAlign: 'center' }}>{pkg.title}</h3>
      )}
...
const WhyChooseUs = () => {
  return (
    <section id="why-choose-us" className="section why-choose-us-section" style={{ padding: '4rem 0' }}>
      <div className="container about-container" style={{ alignItems: 'flex-start' }}>
        <div className="about-image">
          <img src="/about_bg_1778656659879.png" alt="Why Choose Us" className="rounded-image" style={{ width: '100%', height: 'auto', display: 'block' }} />
        </div>

        <div className="about-content why-choose-content">
          <h2 className="section-title text-left" style={{ marginBottom: '1rem', fontSize: '2.5rem' }}>Why Choose <span style={{ color: 'var(--color-primary)' }}>Us</span></h2>
          <p className="about-description" style={{ marginBottom: '2rem', lineHeight: '1.6' }}>
          ...
          <ul className="why-choose-features" style={{ listStyle: 'none', padding: 0, margin: '0 0 2rem 0' }}>
            <li style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', marginBottom: '1.5rem' }}>
              <CheckCircle className="feature-icon" style={{ color: 'var(--color-primary)', flexShrink: 0, marginTop: '0.2rem' }} size={24} />
              <div>
                <h4 style={{ margin: '0 0 0.5rem 0', fontSize: '1.2rem', color: 'var(--color-text)' }}>Creative Excellence</h4>
                <p style={{ margin: 0, color: 'var(--color-text-muted)', fontSize: '0.95rem' }}>Award-winning designs that captivate and engage.</p>
              </div>
            </li>
            <li style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', marginBottom: '1.5rem' }}>
              <CheckCircle className="feature-icon" style={{ color: 'var(--color-primary)', flexShrink: 0, marginTop: '0.2rem' }} size={24} />
              <div>
                <h4 style={{ margin: '0 0 0.5rem 0', fontSize: '1.2rem', color: 'var(--color-text)' }}>Fast Delivery</h4>
                <p style={{ margin: 0, color: 'var(--color-text-muted)', fontSize: '0.95rem' }}>Streamlined processes to meet your deadlines without compromising quality.</p>
              </div>
            </li>
            <li style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
              <CheckCircle className="feature-icon" style={{ color: 'var(--color-primary)', flexShrink: 0, marginTop: '0.2rem' }} size={24} />
              <div>
                <h4 style={{ margin: '0 0 0.5rem 0', fontSize: '1.2rem', color: 'var(--color-text)' }}>Client Focus</h4>
                <p style={{ margin: 0, color: 'var(--color-text-muted)', fontSize: '0.95rem' }}>Your success is our priority. We partner with you every step of the way.</p>
              </div>
            </li>
          </ul>

          <a href="#contact" className="btn btn-primary" style={{ display: 'inline-block', padding: '0.8rem 2rem', fontSize: '1rem', fontWeight: 'bold', borderRadius: '30px' }}>Start Your Project</a>
        </div>
      </div>
    </section>
  );
...
export const Portfolio = ({ openOverlay }) => {
...
// --- Testimonials ---
const testimonials = [
  { name: "MAYUKH BHATTACHARYYA", role: "Customer", rating: 5, quote: "Outstanding designer with excellent service...I ordered for the design and print and they delivered it in 1 day....Never seen this kind of service before....so 100*** from my side and the owner is himself a gentleman..." },
  { name: "Asaph Zoheth Alva", role: "Customer", rating: 5, quote: "The Service was very good and they we're really sincere in their work even better than what i was actually expecting. They got my job done even before the expected time. Truly appreciate the efforts" },
  { name: "champavat krushnarajsinh", role: "Customer", rating: 5, quote: "GRAPHICSPORIUM is consistently exceeded our expectations with their creative and professional graphic design services, delivering visually stunning branding materials that perfectly captured our company's identity while staying true to our vision." },
  { name: "Guru Dronacharya Education", role: "Customer", rating: 5, quote: "I am thoroughly impressed with the exceptional quality of all the products. Each item stands out for its craftsmanship and attention to detail, making it a worthwhile investment. On top of that, the support team is fantastic – they are responsive, friendly, and always ready to assist with any questions or concerns. It’s rare to find such a great combination of quality products and outstanding customer service. I couldn’t be happier and would highly recommend this to anyone!" },
  { name: "Ganesh Giri", role: "Customer", rating: 5, quote: "The design from Graphicsporium exceeded expectations with its creativity and attention to detail. The product quality is top-notch, and the overall experience was seamless. Highly recommended for professional design needs!" },
  { name: "Somnath Seal", role: "Customer", rating: 5, quote: "Graphicsporium delivered outstanding results with creative strategies and flawless execution. Their professional, responsive team brought our vision to life, exceeding expectations. Highly recommend for exceptional advertising and branding services!" }
];
            ...
            <div key={idx} className="testimonial-card glass">
              ...
              <div className="client-info">
                ...
                <div>
                  <h4>{test.name}</h4>
                  <p>{test.role}</p>
                  <div className="client-rating" style={{ display: 'flex', gap: '4px', marginTop: '4px' }}>
...
export const Footer = ({ currentLogo, currentMsmeLogo, onWhatsAppClick }) => {
  return (
    <footer className="footer new-footer">
      <div className="container">
        <div className="new-footer-grid">
          <div className="footer-col">
            ...
            <div style={{ display: 'flex', gap: '15px', marginTop: '20px' }}>
              <a href="https://www.linkedin.com/company/graphicsporium/" target="_blank" rel="noopener noreferrer">
                <img src={iconLinkedin} alt="LinkedIn" className="social-icon" style={{ width: '28px', height: '28px', objectFit: 'contain' }} />
              </a>
              <a href="https://www.instagram.com/_graphicsporium/" target="_blank" rel="noopener noreferrer">
                <img src={iconInsta} alt="Instagram" className="social-icon" style={{ width: '28px', height: '28px', objectFit: 'contain' }} />
              </a>
              <a href="https://www.facebook.com/profile.php?id=61561913078921" target="_blank" rel="noopener noreferrer">
                <img src={iconFb} alt="Facebook" className="social-icon" style={{ width: '28px', height: '28px', objectFit: 'contain' }} />
              </a>
            </div>
          </div>
        ...
        <div style={{ textAlign: 'center', marginTop: '40px', paddingTop: '20px', paddingBottom: '20px', color: 'var(--color-text-muted)', fontSize: '0.9rem', borderTop: '1px solid var(--color-border)' }}>
          ©️ Copyright 2026
        </div>
      </div>
    </footer>
...
// --- Overlays ---
export const ServiceModal = ({ isOpen, onClose, title, onAddToCart }) => {
...
// --- Main App ---
export const ComingSoonOverlay = ({ isOpen, onClose }) => {
...
export const GraphicDesignOverlay = ({ isOpen, onClose }) => {
  useEffect(() => {
    if (isOpen) {
      document.body.style.overflow = 'hidden';
    } else {
  ...
  return (
    <div className="coming-soon-overlay graphic-design-overlay" style={{ display: 'block', overflowY: 'auto', padding: '2rem', zIndex: 9999, backgroundColor: 'var(--color-slate-bg)', position: 'fixed', top: 0, left: 0, width: '100%', height: '100%' }}>
        ...
        {Object.entries(groupedImages).map(([category, imgs]) => (
          <div key={category} style={{ marginBottom: '3rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '1rem' }}>
              {(imgs as string[]).map((src: any, idx: number) => (
...
const MobileCallButton = () => {
  return (
    <a href="tel:+917044495506" className="floating-btn mobile-call-btn mobile-only" aria-label="Call for Free Consultation">
      📞 Free Consultation
    </a>
  );
};

const DesktopConsultationButton = ({ onClick }) => {
  return (
    <button className="floating-btn desktop-consultation-btn desktop-only" onClick={onClick} aria-label="Open Free Consultation">
      📅 Free Consultation
    </button>
  );
};

const ConsultationDrawer = ({ isOpen, onClose }) => {
  return (
    <>
      <div className={`drawer-backdrop ${isOpen ? 'open' : ''}`} onClick={onClose}></div>
      <div className={`consultation-drawer ${isOpen ? 'open' : ''}`}>
        <div className="drawer-header">
          <h3>Free Consultation</h3>
          <button className="close-drawer-btn" onClick={onClose}><X size={24} /></button>
        </div>
        <div className="drawer-content">
          <p>Scan the QR code below to call us directly from your mobile device!</p>
          <div className="qr-container">
            <img src="https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=tel:%2B917044495506" alt="QR Code to call" />
          </div>
          <p style={{ marginTop: '20px', fontSize: '0.9rem', color: 'var(--color-text-muted)' }}>
            Or <Link to="/contact" onClick={onClose} style={{ color: 'var(--color-primary)', textDecoration: 'underline' }}>fill out our contact form</Link> and we'll get back to you!
          </p>
        </div>
      </div>
    </>
...
function App() {
  ...
  const handleAddToCart = (item) => {
    ...
    showToast(`${newCount} item${newCount > 1 ? 's' : ''} added in the cart`);
  };
    ...
    <>
      ...
      <div className="app">
        ...
        <Navbar 
          ...
          onToggleConsultation={() => setIsConsultationOpen(!isConsultationOpen)}
          cartItemCount={estimatorData.length}
        />
        <ConsultationDrawer isOpen={isConsultationOpen} onClose={() => setIsConsultationOpen(false)} />
        <MobileCallButton />
        <DesktopConsultationButton onClick={() => setIsConsultationOpen(true)} />
        <Routes>
          <Route path="/" element={
            <>
          ...
          <Route path="/services" element={<ServicesPage isDarkTheme={isDarkTheme} toggleTheme={() => setIsDarkTheme(!isDarkTheme)} currentLogo={currentLogo} currentMsmeLogo={currentMsmeLogo} onWhatsAppClick={handleWhatsAppRedirect} onAddToCart={handleAddToCart} cartItemCount={estimatorData.length} />} />
          <Route path="/portfolio" element={<PortfolioPage isDarkTheme={isDarkTheme} toggleTheme={() => setIsDarkTheme(!isDarkTheme)} currentLogo={currentLogo} currentMsmeLogo={currentMsmeLogo} onWhatsAppClick={handleWhatsAppRedirect} onAddToCart={handleAddToCart} cartItemCount={estimatorData.length} />} />
          <Route path="/about" element={<AboutUsPage currentLogo={currentLogo} currentMsmeLogo={currentMsmeLogo} onWhatsAppClick={handleWhatsAppRedirect} />} />
          <Route path="/contact" element={<ContactPage currentLogo={currentLogo} currentMsmeLogo={currentMsmeLogo} onWhatsAppClick={handleWhatsAppRedirect} />} />
          <Route path="/blog" element={<Blog currentLogo={currentLogo} currentMsmeLogo={currentMsmeLogo} onWhatsAppClick={handleWhatsAppRedirect} />} />
          <Route path="/blog/:id" element={<BlogPost currentLogo={currentLogo} currentMsmeLogo={currentMsmeLogo} onWhatsAppClick={handleWhatsAppRedirect} />} />
        </Routes>
      </div>
    </>
  );
